...
#                     total_paid = month_payments.aggregate(
#                         total=Sum('amount_paid'))['total'] or Decimal('0')
#
#                     # Determine status
#                     if total_paid >= arrangement.monthly_amount:
#                         status = 'PAID'
#                     elif total_paid > 0:
#                         status = 'PARTIAL'
#                     else:
...
#             messages.success(request, 'Payment arrangement added successfully!')
...
#     arrangement.delete()
#     messages.success(request, 'Payment arrangement has been deleted successfully.')
...
#                     if str(month) in (payment.months_covered or '').split(',')
...
#                 'company': company,
#                 'monthly_status': [],
#                 'monthly_due': Decimal(0),  # Initialize monthly_due
#             }
#
#             arrangements = company.arrangements.filter(is_active=True)
#
...
#                     # Calculate payments for this month
#                     month_payments = payments.filter(month_number=month)
#                     total_paid = month_payments.aggregate(total=Sum('amount_paid'))['total'] or Decimal('0')
#
#                     # Determine status
#                     if total_paid >= arrangement.monthly_amount:
#                         status = 'PAID'
...
#                         status = 'UNPAID'
#
#                     monthly_expected = arrangement.monthly_amount
#                     remaining_in_month = monthly_expected - total_paid
#
#                     month_data = {
#                         'month': month,
#                         'month_name': month_name_str,
...
#                         'paid': total_paid,
#                         'status': status,
#                         'remaining': remaining_in_month,  # Add remaining to month data
#                     }
#                     company_info['monthly_status'].append(month_data)
#
#                     # Check if the current month is within the arrangement's duration
#                     today = datetime.date.today()
#                     current_month = today.month
#                     current_year = today.year
#
#                     target_date = datetime.date(target_year, actual_month, 1)  # Create datetime object for target month
#
#                     if target_date.year == current_year and target_date.month == current_month:
#                         company_info['monthly_due'] += remaining_in_month  # Accumulate monthly due
#
#             companies.append(company_info)
#         return companies
from django.db.models import Sum
...
#             company_info = {
#                 'company': company,
#                 'current_month_balance': Decimal(0),
#             }
#
#             arrangements = company.arrangements.filter(is_active=True)
#             total_to_pay = sum(arr.monthly_amount * arr.number_of_months for arr in arrangements)
#             total_paid = sum(payment.amount_paid for payment in company.payments.all())
#             remaining_balance = total_to_pay - total_paid
#             payment_percentage = (total_paid / total_to_pay * 100) if total_to_pay > 0 else 0
#
#             company_info['total_to_pay'] = total_to_pay
#             company_info['total_paid'] = total_paid
#             company_info['remaining_balance'] = remaining_balance
#             company_info['payment_percentage'] = payment_percentage
#
#             for arrangement in arrangements:
...
#                     payments = company.payments.filter(arrangement=arrangement, month_number=month_diff)
#                     total_paid_this_month = payments.aggregate(total=Sum('amount_paid'))['total'] or Decimal(0)
#                     monthly_due = arrangement.monthly_amount
#                     remaining_in_month = monthly_due - total_paid_this_month
#
#                     company_info['current_month_balance'] += remaining_in_month
#
#             companies.append(company_info)
#         return companies
#
